...
interface TeacherFeedbackForm {
  ...
  scores: {
    ...
    leadership: number; // Additional for superuser
  };
  ...
  // Additional superuser-specific fields
  systemImpactNotes: string;
  adminPerformanceNotes: string;
...
const categoryDescriptions = {
  ...
  leadership: "Leadership qualities and mentoring potential"
};
...
const actionSteps = [
  ...
  "Consider leadership training opportunities",
  "Share best practices with colleagues",
  "Mentor new teachers in the department"
];

export default function SuperUserCreateFeedbackPage() {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">Comprehensive evaluation and system-wide assessment</p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-white/80">Observed by: {observation.observerName}</span>
              </div>
            </div>
          </div>
        </CardHeader>
              ...
              <div className="flex justify-between items-center">
                <div>
                  <Label className="text-base font-medium capitalize flex items-center gap-2">
                    {category === 'leadership' && <Crown className="h-4 w-4 text-purple-500" />}
                    {category.replace(/([A-Z])/g, ' $1').trim()}
                    {category === 'leadership' && <Badge className="bg-purple-100 text-purple-800 text-xs">Superuser</Badge>}
                  </Label>
      ...
      {/* System-Level Analysis (Superuser Specific) */}
      <Card className="border-purple-200 bg-purple-50/30">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Crown className="h-5 w-5 text-purple-600" />
            System-Level Analysis
            <Badge className="bg-purple-100 text-purple-800">Superuser</Badge>
          </CardTitle>
          <p className="text-gray-600">Assessment from organizational perspective</p>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="systemImpact">System Impact & Broader Implications</Label>
            <Textarea
              id="systemImpact"
              value={formData.systemImpactNotes}
              onChange={(e) => setFormData(prev => ({ ...prev, systemImpactNotes: e.target.value }))}
              placeholder="How does this teacher's performance impact the broader educational system? What are the implications for student outcomes, department effectiveness, or school culture?"
              className="min-h-[100px]"
            />
          </div>
          
          <div>
            <Label htmlFor="adminPerformance">Administrative Oversight Assessment</Label>
            <Textarea
              id="adminPerformance"
              value={formData.adminPerformanceNotes}
              onChange={(e) => setFormData(prev => ({ ...prev, adminPerformanceNotes: e.target.value }))}
              placeholder="Assessment of how well the observing administrator conducted the observation and provided guidance..."
              className="min-h-[80px]"
            />
          </div>
          
          <div>
            <Label htmlFor="adminRecommendations">Recommendations for Administrator</Label>
            <Textarea
              id="adminRecommendations"
              value={formData.recommendationsForAdmin}
              onChange={(e) => setFormData(prev => ({ ...prev, recommendationsForAdmin: e.target.value }))}
              placeholder="Specific recommendations for the observing administrator to improve their observation and feedback process..."
              className="min-h-[80px]"
            />
          </div>
        </CardContent>
